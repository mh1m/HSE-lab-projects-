...
class Adagrad(BaseDescent):
    ...
    def update_weights(self, gradient: np.ndarray, iteration: int) -> np.ndarray:
        ...
        # TODO: implement updating weights function
        self.g += gradient ** 2
        diff = (self.eta(iteration) * gradient) / (np.sqrt(self.eps + self.g))
        self.w -= diff
        return diff
...
class LinearRegression:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> LinearRegression:
        ...
        iter = 0
        euclid = self.tolerance
        while iter < self.max_iter and euclid >= self.tolerance:
            self.calc_loss(X, y)
            gradient = self.descent.calc_gradient(X, y)
            w = self.descent.update_weights(gradient, iter)
            euclid = np.linalg.norm(w)
            iter += 1
    ...
    def calc_loss(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        self.loss_history.append(np.square(np.subtract(y, X.dot(self.descent.w))).mean())
